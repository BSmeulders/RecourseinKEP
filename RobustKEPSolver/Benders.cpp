...
cycle_variables_EE EE_Generate_Cycle_Var(IloEnv &env, directedgraph G, const configuration & config, int scenario_number);
...
{
	cout << "Extended Edge Formulation" << endl;
	directedgraph Tested_Graph = G;
	vector<directedgraph> Scenarios;
	if (config.failure_type == 1)
	{
	...
	for (int i = 0; i < config.nr_scenarios; i++)
	{
		cycle_variables_EE cvars = EE_Generate_Cycle_Var(env, Scenarios[i], config, i);
...
cycle_variables_EE EE_Generate_Cycle_Var(IloEnv &env, directedgraph G, const configuration & config, int scenario_number)
{
	...
	// Pre-Processing
	vector<vector<directedarc>> Acopies = DP_Copy(G);
	vector<vector<int>> copy_arc_possible = cycle_preproces_EE(G, Acopies, config.cyclelength); // First index is graph copy, second is arc.
